        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-slate-100 p-8">
                ...
                <div class="mb-8 border-b border-slate-100 pb-6">
                    <h3 class="text-xl font-bold text-slate-800">Informasi Barang Induk</h3>
                    <p class="text-sm text-slate-500 mt-1">Data ini adalah referensi utama (misal: "Laptop Asus ROG").
                        Detail unit fisik akan diinput setelah ini.</p>
                </div>
                ...
                <form action="{{ route('inventaris.store') }}" method="POST">
                    ...
                    <div class="mb-6">
                        <label class="block font-medium text-sm text-slate-700 mb-2">Nama Barang / Aset</label>
                        <input type="text" name="name"
                            class="w-full border-slate-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
                            placeholder="Contoh: Proyektor Epson EB-X500" required autofocus>
                        @error('name') <span class="text-rose-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                    </div>

                    <div class="mb-8">
                        <label class="block font-medium text-sm text-slate-700 mb-2">Deskripsi / Spesifikasi
                            Singkat</label>
                        <textarea name="description" rows="4"
                            class="w-full border-slate-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
                            placeholder="Contoh: Resolusi XGA, 3600 Lumens, HDMI Support"></textarea>
                    </div>
                    ...
                    <div class="flex justify-end items-center gap-4 pt-4 border-t border-slate-100">
                        ...
                        <button type="submit"
                            ...
                            <span>Simpan & Lanjut ke Unit</span>
