        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-slate-100 p-8">
                ...
                <div class="mb-8 border-b border-slate-100 pb-6">
                    <h3 class="text-xl font-bold text-slate-800">Edit Data Unit</h3>
                    <p class="text-sm text-slate-500 mt-1">Perbarui informasi unit kerja atau divisi.</p>
                </div>
                ...
                <form action="{{ route('unit.update', $unit->id) }}" method="POST">
                    ...
                    <div class="mb-8">
                        ...
                        <select name="status"
                            class="w-full border-slate-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 transition-colors">
                            <option value="aktif" {{ old('status', $unit->status) == 'aktif' ? 'selected' : '' }}>Aktif
                            </option>
                            <option value="non-aktif" {{ old('status', $unit->status) == 'non-aktif' ? 'selected' : '' }}>
                                Non-Aktif</option>
                        </select>
                    </div>
