<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2">
            ...
            <span class="text-slate-800">Manajemen Ruangan</span>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                <div>
                    <h3 class="text-2xl font-bold text-slate-800">Daftar Ruangan</h3>
                    <p class="text-slate-500 mt-1">Kelola data ruangan, lokasi, dan status ketersediaan.</p>
                </div>
                    ...
                    <table class="w-full text-sm text-left text-slate-500">
                        <thead class="bg-slate-50 text-xs uppercase text-slate-700 border-b border-slate-100">
                            <tr>
                                ...
                                <th class="px-6 py-4 font-bold">Unit / Prodi</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse($rooms as $room)
                                <tr class="bg-white hover:bg-slate-50 transition-colors">
                                    <td class="px-6 py-4 font-bold text-slate-800 text-base">{{ $room->name }}</td>
                                    <td class="px-6 py-4 text-slate-600">{{ $room->location }}</td>
                                    <td class="px-6 py-4 text-slate-600">{{ $room->unit->name ?? '-' }}</td>
                                    <td class="px-6 py-4">
                                        @if($room->status == 'tersedia')
                                            <span
                                                class="bg-emerald-100 text-emerald-800 text-xs px-2.5 py-1 rounded-full font-bold border border-emerald-200">TERSEDIA</span>
                                        @elseif($room->status == 'digunakan')
                                            <span
                                                class="bg-amber-100 text-amber-800 text-xs px-2.5 py-1 rounded-full font-bold border border-amber-200">DIGUNAKAN</span>
                                        @else
                                            <span
                                                class="bg-rose-100 text-rose-800 text-xs px-2.5 py-1 rounded-full font-bold border border-rose-200">PERBAIKAN</span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            <a href="{{ route('ruangan.show', $room->id) }}"
                                                class="text-emerald-600 hover:text-emerald-800 bg-emerald-50 hover:bg-emerald-100 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors border border-emerald-200 flex items-center gap-1">
